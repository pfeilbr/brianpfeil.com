...
**Token:** `GITHUB_TOKEN` env var takes precedence over `config.local.yaml`, so a
one-off run needs no secret on disk:
`GITHUB_TOKEN=$(gh auth token) go run . -user=pfeilbr -dest=../../content/post`

**Post `description`** comes from the GitHub repo description, run through
`postDescription()` which collapses whitespace and escapes backslashes/quotes —
without that, a description containing a quote breaks the TOML front matter.

**Config structure (`tools/generate-posts/config.yaml`):**
- `repo_filters.include/exclude` — regex patterns to select/skip repos
- `project_repos` — repos categorized as `"project"` rather than `"playground"`
  (finished things vs. scratch space). Anything not listed keeps `"playground"`.
...
- TOML frontmatter with fields: `author`, `categories` (repo language + "playground" or "project"), `date` (repo creation date), `slug`, `tags`, `title`, `repoFullName`, `repoHTMLURL`, `truncated = true`, `summary = " "` (single space)
