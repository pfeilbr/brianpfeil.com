...
SITEMAP = REPO / "static" / "sitemap-courses.xml"
...
def deny_hits(text: str, patterns: list, allow: tuple = ()) -> list[str]:
    """Deny-pattern matches in `text`, after removing the `allow` strings --
    the site's own URL, which contains the owner's name by design."""
    for a in allow:
        if a:
            text = text.replace(a, "")
    hits = []
...
def lesson_description(title: str, n: int, total: int, course: str, about: str) -> str:
    text = f"{title} — lesson {n} of {total} in the free {course} course. {about}"
    return text if len(text) <= 200 else text[:197].rsplit(" ", 1)[0] + "…"


def sitemap(entries: list[dict], site: str) -> str:
    urls = []
    for e in entries:
        for page in e["lessons"] + e["reference"]:
            urls.append(f"  <url><loc>{html.escape(site + page['href'])}</loc><lastmod>{e['updated']}</lastmod></url>")
    return ('<?xml version="1.0" encoding="utf-8" standalone="yes"?>\n'
            '<!-- Generated by tools/courses/sync_courses.py. -->\n'
...
def build_course(course_dir: Path, cfg: dict) -> tuple[dict, dict[str, str]]:
    ...
    about = redact(meta.get("description", ""), cfg["redact"])
    n_lessons = sum(1 for r in sources if r.startswith("lessons/"))
...
def write(entries: list[dict], files: dict[str, dict[str, str]], site: str) -> list[str]:
    changed = []
    ...
    if write_if_changed(SITEMAP, sitemap(entries, site)):
        changed.append("static/sitemap-courses.xml")
    return changed
...
def main(argv: list[str]) -> int:
    ...
    if "--check" in argv:
        ...
        return 0
    changed = write(entries, files, cfg.get("site", "").rstrip("/"))
    for c in changed:
        print(c)
